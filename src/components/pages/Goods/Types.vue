<template>
  <div>
        ...
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item type="text">
              <el-button type="text" @click="handleMultiDel">
                批量删除</el-button
              >
            </el-dropdown-item>
            <el-dropdown-item type="text">
              <el-button type="text" @click="handleMultiUse"
                >批量启用</el-button
              >
            </el-dropdown-item>
            <el-dropdown-item>
              <el-button type="text" @click="handleMultiStopUse"
                >批量禁用</el-button
              >
            </el-dropdown-item>
          </el-dropdown-menu>
    ...
    <!-- 表格 -->
    <el-table stripe :data="types" @selection-change="handleSelection">
      ...
      <el-table-column prop="goodsType" width="150px" label="分类名">
        <template slot-scope="scope">
          <el-input
            ...
            @blur="editSubmit(scope.row)"
          ></el-input>
          <span v-show="!scope.row.editState" @click="handleEdit(scope.row)">{{
            scope.row.goodsType
          }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.goodsTypeState === '已启用' ? 'success' : 'danger'"
            close-transition
            >{{ scope.row.goodsTypeState }}</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            :type="scope.row.editState ? 'success' : 'primary'"
            size="mini"
            icon="edit"
            >{{ scope.row.editState ? "完成" : "编辑" }}</el-button
          >
  ...
  methods: {
    ...
    _multiStateChange(state, cb) {
      if (this.multipleSelection.length == 0) {
        ...
        axios
          .post("api/types/multiStateChange", {
            goodsTypeState: state,
            goodsTypeNo: goodsTypeNo
          })
          .then(cb)
