    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-tag
            :type="
              scope.row.returnStockState === '待审核' ? 'success' : 'danger'
            "
            close-transition
            >{{ scope.row.returnStockState }}</el-tag
          >
        </template>
      </el-table-column>
          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <el-button
                  type="text"
                  :disabled="scope.row.returnStockState === '已完成'"
                  @click="handleStopUse(scope.row)"
                  >完成</el-button
                >
              </el-dropdown-item>
              <el-dropdown-item
                v-show="scope.row.returnStockState === '待审核'"
              >
      ...
      >
        <div class="top">
          <el-form-item label="商品编号" prop="goodsNo">
            <el-input
              v-model="addForm.goodsNo"
              @keydown.enter.native="handleGoodsKey"
              clearable
              placeholder="请输入商品编号"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item>
            <el-button type="primary" @click="handleGoodsSearch"
              >查询商品</el-button
            >
          </el-form-item>
        </div>
        <el-form-item style="width:900px"> </el-form-item>
        <div class="bottom">
        ...
        <el-form-item label="退货原因" prop="returnStockReason">
          <el-input
            v-model="addForm.returnStockReason"
            type="textarea"
            style="width:800px;"
...
export default {
  ...
  methods: {
    ...
    _stateChange(state, returnStockNo, cb) {
      axios
        .post("api/stockReturn/stateChange", {
    ...
    _multiStateChange(state, cb) {
      if (this.multipleSelection.length == 0) {
        ...
        axios
          .post("api/stockReturn/multiStateChange", {
            returnStockState: state,
            returnStockNo: returnStockNo
          })
          .then(cb)
        ...
        if (valid) {
          this._handleSearchInv(this.addForm.goodsNo, res => {
            if (res.data[0].goodsInv < this.addForm.returnStockCount) {
              ...
              let returnAddTime = moment(params.returnAddTime).format(
                "YYYY-MM-DD HH:mm"
              );
              axios
                .post("api/stockReturn/add", {
    ...
    handleSearch() {
      ...
      let returnAddTime = moment(this.filters.returnAddTime).format(
        "YYYY-MM-DD"
      );
      axios
        .post("api/stockReturn/queryByName", {
    ...
    handleCurrentChange(val) {
      this.page = (val - 1) * 10;
      axios
        .post("/api/stockReturn/pagination", {
          page: (val - 1) * 10
        })
        .then(res => {
          this.returnStockInfo = res.data;
