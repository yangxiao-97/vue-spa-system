    ...
    <el-table
      ...
      >
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.stockState === '待审核' ? 'success' : 'danger'"
            close-transition
            >{{ scope.row.stockState }}</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column label="操作" width="230">
        <template slot-scope="scope">
          <el-button type="peimary" size="mini" @click="queryFetch(scope.row)"
            >查看</el-button
          >
          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <el-button
                  type="text"
                  :disabled="scope.row.stockState === '已完成'"
                  size="mini"
  ...
  methods: {
    ...
    _multiStateChange(state, cb) {
      if (this.multipleSelection.length == 0) {
        ...
        multi.map(el => {
          axios
            .post("api/stockSum/multiStateChange", {
              stockState: state,
              stockNo: el.stockNo
            })
            .then(cb)
                ...
                axios
                  ...
                  .then(res => {
                    ...
                    axios
                      .post("api/inventory/update", {
                        goodsInv:
                          parseInt(e.stockCount) +
                          parseInt(res.data[0].goodsInv),
                        goodsNo: e.goodsNo
                      })
                      .catch(err => console.log(err));
                  })
    ...
    addStock() {
      axios.get("api/stockSum/queryAll").then(res => {
    ...
    addSubmit() {
      ...
      if (multi.length === 0) this.$message.warning("请选择商品");
        ...
        axios
          .all([this.addStock(), this.addStockSum(), this.addInventory()])
          .then(
            axios.spread(() => {
