    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <template
            v-else-if="scope.row.goodsInv > 0 && scope.row.goodsInv < 50"
          >
      ...
      <el-table-column label="操作" width="320">
        <template slot-scope="scope">
          <el-button
            v-show="
              (scope.row.goodsInv >= 0 && scope.row.goodsInv < 50) ||
                scope.row.goodsInv <= 0
            "
            type="danger"
            plain
            size="mini"
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      axios
        .post("api/inventory/queryByName", {
