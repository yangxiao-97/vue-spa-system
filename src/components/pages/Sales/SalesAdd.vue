  ...
  <el-row>
    ...
    <el-col :span="16">
      <el-form class="sales-area" :model="addForm">
        <el-table :data="goods" border size="mini" height="260px">
          ...
          <el-table-column
            label="数量"
            prop="goodsSalesCount"
          ></el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="primary" size="mini" @click="regret(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
          ...
          <el-form-item class="vc">
            <el-form-item label="商品编号" label-width="5em">
              <el-input
                ...
                @blur="handleGoodsSearch"
              ></el-input>
            </el-form-item>
            <el-form-item label="数量" label-width="5em">
              <el-input v-model="addForm.goodsSalesCount" clearable> </el-input>
            </el-form-item>
            <el-form-item>
  ...
  methods: {
    ...
    addGoods() {
      if (!this.addForm.goodsNo) {
        ...
        this.goods.forEach(
          el => (salesCountSum += parseInt(el.goodsSalesCount))
        );
    ...
    addGoodsSalesSum() {
      this.goods.map(el => {
        axios
          .post("api/goodsSalesSum/queryById", {
            goodsNo: el.goodsNo
          })
          .then(res => {
            if (res.data.length === 0) {
              axios.post("api/goodsSalesSum/add", {
                ...
                salesProfit:
                  el.deserveMoney -
                  parseFloat(el.stockPrice) * parseInt(el.goodsSalesCount)
              });
            } else {
    ...
    addSubmit() {
      ...
      if (!this.addForm.reciveMoney) {
        ...
        axios.all([this.addSales(), this.addSalesSum(), this.reduceInv()]).then(
          axios.spread(() => {
