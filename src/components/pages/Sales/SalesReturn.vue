          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <el-button
                  type="text"
                  :disabled="scope.row.returnState === '已完成' ? true : false"
      ...
      >
        ...
        <div class="bottom">
          ...
          <el-form-item label="退货数量" prop="returnGoodsCount">
            <el-input v-model="addForm.returnGoodsCount"> </el-input>
          </el-form-item>
        ...
        <el-form-item label="退货原因" prop="returnGoodsReason">
          <el-input
            v-model="addForm.returnGoodsReason"
            type="textarea"
            style="width:800px;"
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      axios
        .post("api/salesReturn/queryByName", {
    ...
    handleReturn(row) {
      axios
        .post("api/salesSum/queryByName", {
          salesNo: row.salesNo,
          salesAddTime: null
        })
        .then(res => {
          axios
            .post("api/goods/queryById", {
              goodsNo: row.goodsNo
            })
            .then(e => {
              let oldSalesSum = res.data[0].salesMoneySum;
              let newSalesSum =
                parseInt(oldSalesSum) - parseInt(row.returnGoodsMoney);
              let returnGoodsProfit =
                parseInt(row.returnGoodsCount) * parseInt(e.data[0].stockPrice);
              let oldSalesProfit = res.data[0].salesProfitSum;
              let newSalesProfit =
                parseInt(oldSalesProfit) - parseInt(returnGoodsProfit);
              axios
                .post("api/salesSum/update", {
    ...
    handleCurrentChange(val) {
      this.page = (val - 1) * 10;
      axios
        .post("/api/salesReturn/pagination", {
          page: (val - 1) * 10
        })
        .then(res => {
          this.returnGoodsInfo = res.data;
