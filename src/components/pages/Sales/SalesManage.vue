      ...
      >
        ...
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="queryFetch(scope.row)"
              >查看</el-button
            >
            <el-button type="danger" size="mini" @click="handelDel(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleMultiStateChange(salesNo, cb) {
      axios
        .post("api/salesSum/multiStateChange", {
          salesNo: salesNo
        })
        .then(cb)
        .catch(err => console.log(err));
    ...
    queryCompleted() {
      axios
        .get("api/salesSum/queryCompleted")
        .then(res => {
